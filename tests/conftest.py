...
def synthetic_dataset_dir(tmp_path: Path) -> Tuple[Path, Path, Path, Path]:
    ...
    for i in range(5):
        ...
        for band in S1_BAND_NAMES:
            dim = NATIVE_SHAPES[band]  # 120
            # SAR backscatter in dB: [-25 to -5]
            arr = np.random.uniform(-25.0, -5.0, size=(dim, dim)).astype(np.float32)
            tifffile.imwrite(str(patch_s1_dir / f"{s1_name}_{band}.tif"), arr)
        ...
        for band in S2_BAND_NAMES:
            dim = NATIVE_SHAPES[band]  # 120, 60, or 20
            # Optical reflectance values (DN 0 - 4000)
            arr = np.random.uniform(200.0, 3500.0, size=(dim, dim)).astype(np.float32)
            tifffile.imwrite(str(patch_s2_dir / f"{patch_id}_{band}.tif"), arr)
        ...
        row = {
            ...
            "output": "Yes, water body is present." if i % 2 == 0 else "No water body.",
            "type": "binary" if i % 2 == 0 else "mcq",
            "category": "presence",
            "split": "train" if i < 3 else ("validation" if i == 3 else "test"),
